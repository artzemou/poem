...
function search(str, shadow) {
    ...
    $('p, pre').each(function() {
        ...
        // var pattern = /([0-9-A-Za-zÀ-ÖØ-öø-ÿ_ /]+[^<sup[*^/>]+[^<sub[*^/>]+[^<span[*^/>])/gi; // wrap words
        // // var pattern = /^([^<>%$])/g
...
// function getMarkInnerText() {
//     var sel = rangy.getSelection(), ids = [];
//     for (var r = 0, range, marks; r < sel.rangeCount; ++r) {
//         range = sel.getRangeAt(r);
//         if (range.startContainer == range.endContainer && range.startContainer.nodeType == 3) {
//             range = range.cloneRange();
//             range.selectNode(range.startContainer.parentNode);
//         }
//         marks = range.getNodes([1], function(node) {
//               return node.nodeName.toLowerCase() == 'mark' || node.nodeName.toLowerCase() == 'b';
//         });

//     }
//     console.log(marks)
//     if(marks.length === 1) return marks[0].innerText; 
